...
import { saveAdvancedInquiry } from "@/actions/customer";
...
export default function AddInquiryWizard() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Checkboxes are omitted if unchecked, handle them:
    data.loanRequired = formData.get("loanRequired") ? "true" : "false";
    data.exchangeRequired = formData.get("exchangeRequired") ? "true" : "false";

    await saveAdvancedInquiry(data);
    // No setLoading(false) needed because server action redirects directly
  };

  return (
    <div style={{ paddingBottom: '64px' }}>
      <h2 style={{ marginBottom: '32px' }}>Smart Customer Intake</h2>
      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: '800px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
        ...
        <div className="glass-card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
              ...
              <label>Deal Stage
                <select name="stage" style={inputStyle} defaultValue="Inquiry" disabled>
                  <option value="Inquiry">Inquiry (Auto-set)</option>
                </select>
              </label>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
              <label>Next Follow-Up Date <input type="date" name="nextFollowUp" style={inputStyle} /></label>
              <label>Initial Inquiry Notes <input name="notes" required style={inputStyle} placeholder="Specific requirements or remarks" /></label>
            </div>
          </div>
        </div>
        ...
        <button type="submit" disabled={loading} className="btn-primary" style={{ padding: '16px 32px', fontSize: '1.1rem', marginTop: '16px', width: '100%' }}>
          {loading ? "Processing Engine..." : "Generate Smart Matches"}
        </button>
      </form>
    </div>
  );
